...
\begin{theorem}[Distribution of the standardized sample mean]
    Let \({(X_i)}_{i=1}^n \iid \mathscr{N}(\mu, \sigma^2)\). Then,\[
...
\fbox{
\begin{minipage}[t]{.76\textwidth}
    \textbf{Nerd Interjection!} In case you missed it, here's the gamma kernel trick:\[
    \int_0^\infty \frac{1}{\upGamma(\alpha)\cdot \beta^\alpha} x^{\alpha -1} \e^{-\frac{x}{\beta}} \der x = 1,
    \]so multiplying both sides by \(\upGamma(\alpha) \cdot \beta^\alpha\) gives us\[
        \int_0^\infty  x^{\alpha -1} \e^{-\frac{x}{\beta}}\der x = \upGamma(\alpha) \cdot \beta^\alpha
    \]no matter what the values of \(\alpha\) and \(\beta\) are. 
\end{minipage}}
\vspace{10pt}
...
We can now prove theorem 1.3.1:\begin{proof}
...
Given these two \(\upchi^2\) random variables, let \(u = \frac{x_1 / k_1}{x_2 / k_2}\) and \(v = x_2\). 
...
\begin{theorem}[Distribution of the \(i^\text{th}\) order statistic]
...
Suppose that the \(i^\text{th}\) order statistic \(X_{(i)}\) has value \(x\), and that all order statistics are independent. 
